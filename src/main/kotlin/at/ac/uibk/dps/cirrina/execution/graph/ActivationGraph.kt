  ...
  companion object {
    fun create(stateMachines: Collection<StateMachine>): ActivationGraph {
      return ActivationGraph().apply {
        stateMachines.forEach { addVertex(it.name) }
        ...
        for (source in stateMachines) {
          ...
          for (event in outputs) {
            for (target in stateMachines) {
              ...
              // TODO: This cannot be matched at this point because a raise action has an
              // expression
              //  as target, unless we find a way to evaluate it here we cannot filter out
              // targeted
              //  events here
